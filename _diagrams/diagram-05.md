...
order: 5
slide_ref: "24-25"
description: "The 3-stage Dockerfile workflow: compile → train (record + create) → runtime with AOT cache."
...
**Spring Boot specific:** Unlike frameworks with single-property AOT support, Spring Boot requires explicit `-XX:AOTMode` steps. This gives you full control over the training run but requires a more complex Dockerfile.
